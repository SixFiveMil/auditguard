"""
AuditGuard - Authorized Security Research & Scope Oversight CLI.
Interactive management interface for authorized bug bounty and VDP engagements.
"""
...
from core.scope_validator import ScopeValidator, ScopeViolationError, OutOfScopePathError
...
def load_config() -> Dict[str, Any]:
    ...
    config_path = Path("config/programs.yaml")
...
def cmd_scope(args: Any) -> None:
    cfg = load_config()
    active_key, validator, prog_cfg = get_active_validator(cfg)
    print("\n" + "=" * 60)
...
def cmd_check(args: Any) -> None:
    cfg = load_config()
    _, validator, _ = get_active_validator(cfg)
    url = args.url
    print(f"\nChecking URL: {url}")
    try:
...
def cmd_probe(args: Any) -> None:
    ...
    audit_logger = AuditLogger("audit/audit_log.jsonl")
...
def cmd_logs(args: Any) -> None:
    logger = AuditLogger("audit/audit_log.jsonl")
